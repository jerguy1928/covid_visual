...
//create basemap
	...
	reset_zoom = L.easyButton('<i class="fas fa-home"></i>', 
	function refresh(){map.setView([40, -15], 2);}).addTo(map);
	
	reset_zoom.setPosition('bottomleft');
...
// add search button
	...
	search_btn = L.control.geocoder('ge-6071d21825e9421c',options);
	search_btn.setPosition('bottomleft')
...
// determines scale for each field of data (e.g. total cases, new cases, etc.)
	function get_grades(field){
		return  field == "rank" 	       ? [200,150,100,50,25,10,1]:
				field == "total_cases"     ? [1000000,100000,50000,10000,1000,100,1]:
...
// sets hover information and style information based on field
	function get_fields(props){
		...
		var hover_val = props[field];
		var hover_num = props[field];
		grade = get_grades(field);
		...
		switch(document.getElementById('numbers').value){
			...
			case "total_recovered":
				...
				if (hover_val == 'N/A' | hover_val == 'NULL'){
					...
					hover_num = props.total_recovered/props.total_cases;
				}
				break;
			...
			case "cases_1m":
				...
				if (hover_val == 'NULL'| props.name == 'Diamond Princess' | props.name == 'MS Zaandam'){
		...
		var style_val = color_assign(hover_num,grade,color_ramp(field));
